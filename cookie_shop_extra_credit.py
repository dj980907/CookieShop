...
def bake_cookies(filepath):
  ...
  f = open(filepath, 'r')
  cookies_csv = f.readlines()
  cookies_csv = cookies_csv[1:]
  cookie_list = []
  for cookie in cookies_csv:
    cookie_dict = {}
    id = cookie.split(',')[0]
    title = cookie.split(',')[1]
    description = cookie.split(',')[2]
    price = cookie.split(',')[3]
    cookie_dict['id'] = int(id)
    cookie_dict['title'] = title
    cookie_dict['description'] = description
    cookie_dict['price'] = float(price.strip('$'))
    if len(cookie.split(',')) == 5:
      if cookie.split(',')[4].strip() == 'sugar free':
        cookie_dict['sugar_free'] = True
      elif cookie.split(',')[4].strip() == 'gluten free':
        cookie_dict['gluten_free'] = True
      elif cookie.split(',')[4].strip() == 'contains nuts':
        cookie_dict['contains_nuts'] = True
    cookie_list.append(cookie_dict)
  #print(cookie_list)#juct to check
  return cookie_list
...
def welcome():
  ...
  not_want_nuts = input("Are you allergic to nuts? (Yes or No) ")
  gluten_free = input("Are you allergic to gluten? (Yes or No) ")
  sugar_free = input("Do you suffer from diabetes? (Yes or No) ")
  if not_want_nuts.lower == 'yes' and gluten_free.lower == 'yes' and sugar_free.lwer == 'yes':
      return 'yesyesyes'
  elif not_want_nuts.lower == 'yes' and gluten_free.lower == 'yes' and sugar_free.lwer == 'no':
      return 'yesyesno'
  elif not_want_nuts.lower == 'yes' and gluten_free.lower == 'no' and sugar_free.lwer == 'yes':
      return 'yesnoyes'
  elif not_want_nuts.lower == 'no' and gluten_free.lower == 'yes' and sugar_free.lwer == 'yes':
      return 'noyesyes'
  elif not_want_nuts.lower == 'yes' and gluten_free.lower == 'no' and sugar_free.lwer == 'no':
      return 'yesnono'
  elif not_want_nuts.lower == 'no' and gluten_free.lower == 'yes' and sugar_free.lwer == 'no':
      return 'noyesno'
  elif not_want_nuts.lower == 'no' and gluten_free.lower == 'no' and sugar_free.lwer == 'yes':
      return 'nonoyes'
  elif not_want_nuts.lower == 'no' and gluten_free.lower == 'no' and sugar_free.lwer == 'no':
      return 'nonono'
...
def display_cookies(cookies):
  ...
  sugar_free_cookies = []
  gluten_free_cookies = []
  contains_nuts_cookies = []
  for cookie in cookies:
    for key in cookie.keys():
      if 'sugar_free' in key:
        sugar_free_cookies.append(cookie)
      elif 'gluten_free' in key:
        gluten_free_cookies.append(cookie)
      elif 'contains_nuts' in key:
        contains_nuts_cookies.append(cookie)
  print("Great! Here are the cookies without {} that we think you might like:\n".format('sugar'))
  for cookie in sugar_free_cookies:
    message = "#{} - {}\n{}\nPrice: ${}\n".format(cookie['id'], cookie['title'], cookie['description'], format(float(cookie['price']),'.2f'))
    print(message)
  print("Great! Here are the cookies without {} that we think you might like:\n".format('gluten'))
  for cookie in gluten_free_cookies:
    message = "#{} - {}\n{}\nPrice: ${}\n".format(cookie['id'], cookie['title'], cookie['description'], format(float(cookie['price']),'.2f'))
    print(message)
  print("Great! Here are the cookies without {} that we think you might like:\n".format('nuts'))
  for cookie in contains_nuts_cookies:
    message = "#{} - {}\n{}\nPrice: ${}\n".format(cookie['id'], cookie['title'], cookie['description'], format(float(cookie['price']),'.2f'))
    print(message)
...
def solicit_quantity(id, cookies):
  ...
  is_valid = False
  while not is_valid:
    num_of_cookies = input("My favorite! How many {} would you like? ".format(cookie_title))
    if num_of_cookies.isnumeric():
      is_valid = True
  subtotal = format(int(num_of_cookies) * float(cookie_price), '.2f')
  print("Your subtotal for {} {} is ${}.".format(num_of_cookies, cookie_title, subtotal))
  return int(num_of_cookies)
...
def display_order_total(order, cookies):
  ...
  for cookie in order:
    ...
    title = cookies[cookie['id']-1]['title']
    print("-{} {}".format(quantity, title))
    price = cookies[cookie['id']-1]['price'] * quantity
    total += price
  total = format(total, '.2f')
  # print(total) #just to check
